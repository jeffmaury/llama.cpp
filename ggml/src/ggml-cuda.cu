...
static void * ggml_backend_cuda_get_proc_address(ggml_backend_reg_t reg, const char * name) {
    GGML_UNUSED(reg);
    if (strcmp(name, "ggml_backend_cuda_split_buffer_type") == 0) {
        return (void *)ggml_backend_cuda_split_buffer_type;
    }
    if (strcmp(name, "ggml_backend_cuda_register_host_buffer") == 0) {
        return (void *)ggml_backend_cuda_register_host_buffer;
    }
    if (strcmp(name, "ggml_backend_cuda_unregister_host_buffer") == 0) {
...
ggml_backend_t ggml_backend_cuda_init(int device) {
    ...
    ggml_backend_cuda_reg_context * reg_ctx = (ggml_backend_cuda_reg_context *)ggml_backend_cuda_reg()->context;

    ggml_backend_t cuda_backend = new ggml_backend {
        /* .guid      = */ ggml_backend_cuda_guid(),
        /* .interface = */ ggml_backend_cuda_interface,
        /* .context   = */ ctx,
        /* .device    = */ reg_ctx->devices[device]
    };
